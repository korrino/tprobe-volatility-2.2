...
class HeisenbergVolatilityDTB(obj.VolatilityMagic):
...
class AbstractHeisenbergPlugin(object):
    ...
    def __init__(self, heisenberg_core, name = None):
        self.core = heisenberg_core
        self.functions = heisenberg_core.functions
...
class AbstractHeisenbergApiFunction(AbstractHeisenbergPlugin):
    def execute(self, *args, **kwargs):
        return self.calculate(*args, **kwargs)


class AbstractVolatilityPluginWrapper(AbstractHeisenbergPlugin):
    def __init__(self, heisenberg_core, config, plugin_class, *args, **kwargs):
        AbstractHeisenbergPlugin.__init__(self, heisenberg_core, name = plugin_class.__name__.lower())
...
#        self.bpts = {}
    ...
    def load_volatility_plugins(self):
        ...
        for plugin_name in cmds:
            if cmds[plugin_name].is_valid_profile(self.addrspace.profile) and plugin_name is not 'heisenberg':
    ...
    def load_heisenberg_plugins(self):
        cmds = registry.get_plugin_classes(AbstractHeisenbergPlugin, lower = True)
        for plugin_name in cmds:
