...
class HeisenbergShell(object):
    def __init__(self, gshell):
        ...
        self.output = gtk.TextView(self.textBuffer)
        ...
        vbox.pack_start(self.swh, True)
        vbox.pack_start(self.input, False)
...
class RegistersView(object):
    def __init__(self, gshell):
        ...
        hbox.pack_start(self.regs1view, False, 0)
        hbox.pack_start(self.regs3view, False, 0)
...
class CodeView(object):
    ...
    def key_pressed(self, widget, data):
#        self.gshell.log(str(data.keyval)+"\n")
        if(gtk.gdk.keyval_name(data.keyval) == "F2"):
            self.activate_insert_bp(widget, data)
        if(gtk.gdk.keyval_name(data.keyval) == "F8"):
            self.gshell.functions.ni.calculate()
        if(gtk.gdk.keyval_name(data.keyval) == "F7"):
            self.gshell.functions.si.calculate()
        if(gtk.gdk.keyval_name(data.keyval) == "F9"):
            self.gshell.functions.c.calculate()
        if(gtk.gdk.keyval_name(data.keyval) == "g"):
...
#                liststore.append([self.gshell.functions.get_process_name(proc), "%08x" % proc.v(), "blue"])
                color = "blue"
            liststore.append([self.gshell.functions.get_process_name(proc), "%08x" % proc.v(), color])
        return liststore
...
class Main(object):
    def __init__(self, gshell):
        ...
        # Heisenberg view
        self.hButton = gtk.Button("Heisenberg Shell")
        self.hButton.connect("clicked", self.toggle, (self.gshell.hs, HeisenbergShell))

        self.quitButton = gtk.Button("Exit GShell")
        self.quitButton.connect("clicked", self.quit)

        self.vbox = gtk.VBox(False, 0)
        self.vbox.pack_start(self.mbButton, True)
        self.vbox.pack_start(self.mdButton, True)
        self.vbox.pack_start(self.rButton, True)
        self.vbox.pack_start(self.cButton, True)
        self.vbox.pack_start(self.pButton, True)
        self.vbox.pack_start(self.hButton, True)
        self.vbox.pack_start(self.quitButton, True)
...
class GtkConsole(heisenberg.AbstractHeisenbergPlugin):
    name = 'w32gshell'
